...
export class DeployableWandInstance extends Phaser.Physics.Arcade.Sprite {
    private wand: Wand;
    ...
    public static create(scene: Scene, shooter: any, room: Room): void {
        const wand = shooter.getDeployableWand();
        if (!wand) return;
        const spawnPoint = this.getSpawnPoint(scene, shooter);
        if (!spawnPoint) return;
        new DeployableWandInstance(scene, spawnPoint.x, spawnPoint.y, wand as DeployableWand, shooter, room);
    }

    constructor(scene: Phaser.Scene, x: number, y: number, wand: DeployableWand, shooter: any, room: Room) {
        super(scene, x, y, wand.config.displayConfig?.sprite || 'spell-bot');
        this.setScale(1.5);
    ...
    private createAnimation() {
        ...
        this.scene.anims.create({
            ...
            frames: this.scene.anims.generateFrameNumbers(this.wand.config.displayConfig?.animation || 'turret', {
                start: 0,
                end: 3
            }),
            frameRate: 10,
    ...
    preUpdate(time: number, delta: number) {
        ...
        if (this.targetEnemy && this.wand) {
            this.wand.fireAtTarget(this, this.targetEnemy);
        }
    ...
    private respawn() {
        ...
        const spawnPoint = DeployableWandInstance.getSpawnPoint(mainScene, this.shooter);
        if (!spawnPoint) return;

        // Create new instance
        new DeployableWandInstance(this.scene, spawnPoint.x, spawnPoint.y, this.wand as DeployableWand, this.shooter, this.room);
