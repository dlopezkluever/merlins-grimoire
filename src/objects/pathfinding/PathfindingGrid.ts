...
export class PathfindingGrid {
  ...
  private createBaseGrid(map: Tilemaps.Tilemap): number[][] {
    ...
    if (!wallsLayer) {
      ...
      // Return empty passable grid if no walls layer
      for (let y = 0; y < map.height; y++) {
        grid[y] = [];
        for (let x = 0; x < map.width; x++) {
          grid[y][x] = 0; // All passable
        }
      }
      return grid;
    }

    // Create grid based on wall tiles
    for (let y = 0; y < map.height; y++) {
      grid[y] = [];
      for (let x = 0; x < map.width; x++) {
        const tile = wallsLayer.tilemapLayer.getTileAt(x, y);
        // 0 = walkable, 1 = wall
        grid[y][x] = tile && tile.collides ? 1 : 0;
      }
    }
